...
- **Project** — `has_one_attached :featured_image`, `has_many_attached :screenshots`,
  ...
  project page's "What I built" list, one point per line (`highlight_list`). For every project in
  `projects.yml` the file wins: every `showcase:load` (a deploy step) overwrites admin edits to
  their tags, description and highlights, so edit those in the file. The older projects' images
  and links stay admin-managed. Scopes `featured`,
  ...
  `SECTIONS` order; each project page reuses that hero's layout. `has_links?` checks for a GitHub
  or live URL.
